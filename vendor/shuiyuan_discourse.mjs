...
const DEFAULT_IMAGE_CACHE_DIR = "./.shuiyuan-cache/shuiyuan-discourse/image-cache";
const DEFAULT_AUTH_PATH = "~/.shuiyuan-discourse/auth.json";
const DEFAULT_AUTH_PENDING_PATH = "~/.shuiyuan-discourse/auth.pending.json";
...
function printJson(payload) {
  // Callers may emit large topic payloads and main() exits immediately after
  // this returns. A synchronous write prevents redirected/pipe output from
  // being truncated before Node flushes stdout.
  fs.writeFileSync(process.stdout.fd, `${JSON.stringify(payload, null, 2)}\n`);
}
...
function flattenCredentialResult(result) {
...
function loadFileCredential(site, filePath, source) {
  ...
  const flat = flattenCredentialResult(found);
  if (!flat) return null;
  return {
    site: normalizeSite(site),
    user_api_key: flat.key,
    user_api_client_id: flat.client_id,
    source,
  };
}

function loadAllFileCredentials(site, filePath, source) {
  const raw = readJsonFile(filePath);
  if (!raw) return [];
  const found = extractCredentialForSite(raw, site);
  if (!found) return [];
  const entries = Array.isArray(found) ? found : [found];
  return entries.map((e) => ({
    site: normalizeSite(site),
    user_api_key: e.key,
    user_api_client_id: e.client_id,
    source,
  }));
}
...
const DEFAULT_ROTATION_STATE_PATH = "~/.shuiyuan-discourse/rotation-state.json";

function loadRotationState(statePath) {
  try {
...
function saveRotationState(statePath, state) {
  const dir = path.dirname(expandUserPath(statePath));
  fs.mkdirSync(dir, { recursive: true });
  writeJsonFileSecure(statePath, state);
}

function pickCredentialRoundRobin(creds, statePath, site) {
  if (creds.length <= 1) return creds[0] || null;
  const state = loadRotationState(statePath);
  const normalized = normalizeSite(site);
  const idx = (state[normalized] || 0) % creds.length;
  const picked = creds[idx];
  state[normalized] = (idx + 1) % creds.length;
  saveRotationState(statePath, state);
  return picked;
}

function resolveCredentialWithRotation({ site, authFile, rotationStateFile }) {
  const envCred = loadEnvCredential(site);
  if (envCred) return { credential: envCred, total: 1, index: 0, _allCredentials: null };

  const allCreds = loadAllFileCredentials(site, authFile, "auth_file");
  if (allCreds.length === 0) {
    throw new CredentialError(
      "No User API credential found. Run 'auth init' (get link) then 'auth finish', or run 'auth import', or set SHUIYUAN_USER_API_KEY.",
    );
  }
  const picked = pickCredentialRoundRobin(allCreds, rotationStateFile, site);
  return { credential: picked, total: allCreds.length, index: allCreds.indexOf(picked), _allCredentials: allCreds };
}
...
async function requestJsonWithRotation(site, requestPath, params, cred, allCredentials, runtime, timeout) {
  try {
    return await requestJson(site, requestPath, params, cred, runtime, timeout);
  } catch (err) {
    if (err instanceof HttpRequestError && err.status === 429 && allCredentials && allCredentials.length > 1) {
      const otherCreds = allCredentials.filter((c) => c.user_api_key !== cred.user_api_key);
      for (const next of otherCreds) {
        try {
          const retried = await requestJson(site, requestPath, params, next, runtime, timeout);
          retried.rotation_retry = { original_key_prefix: cred.user_api_key?.slice(0, 8), retried_with_key_prefix: next.user_api_key?.slice(0, 8) };
          return retried;
        } catch (retryErr) {
          if (retryErr instanceof HttpRequestError && retryErr.status === 429) continue;
          throw retryErr;
        }
      }
    }
...
function addCredentialToAuthFile(filePath, site, key, clientId) {
  let raw = readJsonFile(filePath) || {};
  const normalized = normalizeSite(site);
  const newPair = {
    site: normalized,
    user_api_key: key,
  ...
  if (Array.isArray(raw.auth_pairs)) {
    if (raw.auth_pairs.some((p) => p.site === normalized && p.user_api_key === key)) {
      throw new SkillError("This key already exists in auth_pairs for this site.");
    }
    raw.auth_pairs.push(newPair);
  } else if (raw.site && raw.user_api_key) {
    const existingPair = { site: raw.site, user_api_key: raw.user_api_key, user_api_client_id: raw.user_api_client_id || null };
    if (existingPair.user_api_key === key) {
      throw new SkillError("This key is already the current active key.");
    }
    raw.auth_pairs = [existingPair, newPair];
    delete raw.site;
    delete raw.user_api_key;
    delete raw.user_api_client_id;
  } else {
    raw = { ...newPair };
  }

  raw.updated_at = utcNowIso();
  writeJsonFileSecure(filePath, raw);
  return raw;
}

function removeCredentialFromAuthFile(filePath, site, key) {
  const raw = readJsonFile(filePath);
  if (!raw) throw new SkillError("No auth file found.");

  if (Array.isArray(raw.auth_pairs)) {
    const idx = raw.auth_pairs.findIndex(
      (p) => normalizeSite(p.site) === normalizeSite(site) && p.user_api_key === key,
    );
    if (idx === -1) throw new SkillError("Key not found in auth_pairs.");
    raw.auth_pairs.splice(idx, 1);
    if (raw.auth_pairs.length === 0) {
      throw new SkillError("Cannot remove the last remaining key. Use 'auth import' to replace it.");
    }
  } else if (raw.user_api_key === key) {
    throw new SkillError("Cannot remove the only key. Use 'auth import' to replace it.");
  } else {
    throw new SkillError("Key not found.");
  }
...
async function handleAuthStatus(ctx) {
  ...
  const allCreds = loadAllFileCredentials(ctx.site, ctx.authFile, "auth_file");
  const authCred = allCreds.length > 0 ? allCreds[0] : null;
  const resolved = envCred || authCred;

  const rotationState = loadRotationState(ctx.rotationStateFile);
  const normalized = normalizeSite(ctx.site);
  const rotationIdx = rotationState[normalized] || 0;
  ...
  if (allCreds.length > 1) {
    result.key_rotation = {
      enabled: true,
      total_keys: allCreds.length,
      next_index: rotationIdx % allCreds.length,
      keys: allCreds.map((c, i) => ({
        index: i,
        key_prefix: c.user_api_key ? c.user_api_key.slice(0, 8) + "..." : null,
        source: c.source,
      })),
    };
  } else if (allCreds.length === 1) {
    result.key_rotation = { enabled: false, total_keys: 1, hint: "Use 'auth add --user-api-key <key>' to add more keys for rotation." };
  }
...
async function handleAuthAdd(ctx, args) {
  if (!args.user_api_key) {
    throw new SkillError("Missing required argument: --user-api-key");
  }

  const updated = addCredentialToAuthFile(ctx.authFile, ctx.site, args.user_api_key, args.client_id || null);
  const pairCount = Array.isArray(updated.auth_pairs) ? updated.auth_pairs.length : 1;
  return {
    ok: true,
    site: ctx.site,
    total_keys: pairCount,
    saved: {
      path: ctx.authFile,
      has_client_id: Boolean(args.client_id),
      updated_at: utcNowIso(),
...
async function handleAuthRemove(ctx, args) {
  if (!args.user_api_key) {
    throw new SkillError("Missing required argument: --user-api-key");
  }

  const updated = removeCredentialFromAuthFile(ctx.authFile, ctx.site, args.user_api_key);
  const pairCount = Array.isArray(updated.auth_pairs) ? updated.auth_pairs.length : 1;
  return {
    ok: true,
    site: ctx.site,
...
function requireCredential(ctx) {
  const result = resolveCredentialWithRotation({
    site: ctx.site,
    authFile: ctx.authFile,
    rotationStateFile: ctx.rotationStateFile,
  });
  if (result.total > 1) {
    result.credential._rotation = { total: result.total, index: result.index };
    ctx._allCredentials = result._allCredentials;
  }
...
async function handleSearch(ctx, args) {
  ...
  const result = await requestJsonWithRotation(
    ctx.site,
    "/search.json",
    {
      q: args.query,
...
async function handleCategories(ctx) {
  const cred = requireCredential(ctx);
  const result = await requestJsonWithRotation(ctx.site, "/site.json", null, cred, ctx._allCredentials, ctx.runtime, ctx.timeout);
...
async function execute(ctx, parsedCommand) {
  switch (parsedCommand.kind) {
    ...
    case "auth_add":
      return handleAuthAdd(ctx, parsedCommand.args);
    case "auth_remove":
      return handleAuthRemove(ctx, parsedCommand.args);
    case "search":
