...
export function initSessionView({ onOpened } = {}) {
  ...
  // The worker owns one active session at a time; remember which one this
  // surface is showing so a follow-up can't be sent to a session that another
  // window switched to underneath us.
  let sessionId = null;
  let livePoll = null;
  ...
  function chatStatus(text, { stoppable = false } = {}) {
  ...
  function showResult(state) {
    sessionId = state.historyId || null;
    ...
    chatStatus(state.chatBusy ? "回答生成中…" : null, { stoppable: true });
    if (state.chatBusy) startLivePoll();
    if (state.historyError) {
      notice(`报告已生成，但保存到历史记录失败：${state.historyError}`, "warn");
    }
  }
  ...
  // The worker can be recycled mid-search or mid-follow-up (browser update,
  // memory pressure), in which case no sy.searchDone / sy.chatDone ever arrives.
  // Poll while something is in flight so the view eventually settles instead of
  // spinning forever.
  function startLivePoll() {
    ...
    livePoll = setInterval(async () => {
      try {
        ...
        if (state.status !== "running" && !state.chatBusy) {
          stopLivePoll();
          await refresh();
          renderHistory();
        }
      } catch {
        // Worker unreachable; keep polling — it may come back.
      }
    }, 10_000);
  }
  ...
  /**
   * Only CHECK the host permission here — never request it.
   * chrome.permissions.request() must run inside a live user gesture, and by
   * this point we have already awaited storage; in a popup the prompt would also
   * dismiss the popup and abandon the request. The setup page asks for it right
   * inside its click handler, which is the one place that works.
   */
  async function ensureLlmPermission() {
    ...
    try {
      ...
      throw new Error("LLM Base URL 不是合法网址，请到设置页修正。");
    }
    const has = await chrome.permissions.contains({ origins: [`${origin}/*`] });
    if (!has) {
      throw new Error(`缺少访问 ${origin} 的权限：请打开设置页，点击「保存配置」以授予权限。`);
    }
  }
  ...
  async function run() {
    ...
    if (!topic) return;
    notice("");
    // Disable BEFORE the round trip: otherwise a double-click sends two starts
    // and the worker would run the search twice.
    setRunning(true);
    try {
      ...
      // A rejected start means the worker's state may differ from ours.
      refresh().catch(() => {});
    }
  }
  ...
  async function ask() {
    ...
    try {
      await ensureLlmPermission();
      await send({ type: "sy.chat.ask", question, sessionId });
      input.value = "";
      bubble("user", question);
      chatStatus("回答生成中…", { stoppable: true });
      startLivePoll();
    } catch (err) {
      notice(err.message || String(err));
    } finally {
  ...
  chrome.runtime.onMessage.addListener((msg) => {
    if (msg?.type === "sy.searchEvent") {
    ...
    } else if (msg?.type === "sy.storageWarning") {
      notice(`保存到本地存储失败：${msg.error}（历史记录/凭证可能未能写入）`, "warn");
    } else if (msg?.type === "sy.chatEvent") {
      const event = msg.event;
      if (event.type === "chat-thinking") chatStatus("回答生成中…", { stoppable: true });
      if (event.type === "chat-search") {
        const prefix = event.round > 1 ? `第 ${event.round} 轮追加搜索` : "材料不足，追加搜索";
        chatStatus(`${prefix}：${event.queries.join("、")}`, { stoppable: true });
      }
      if (event.type === "chat-trimmed") {
        chatStatus(`对话过长，已裁剪较早的 ${event.dropped} 条追问记录…`, { stoppable: true });
      }
      if (event.type === "llm-retry") {
        chatStatus(
          event.reason === "empty"
            ? `模型返回空回复，正在重试…`
            : `LLM 请求失败，${Math.round((event.wait || 0) / 1000)}s 后重试…`,
          { stoppable: true },
        );
      }
      if (event.type === "rate-limited") {
        chatStatus(`论坛限流，等待 ${event.wait}s…`, { stoppable: true });
      }
      ...
      if (msg.ok) {
        if (msg.historyError) {
          notice(`回答已生成，但保存到历史记录失败：${msg.historyError}`, "warn");
        }
        send({ type: "sy.search.state" })
          .then((state) => {
