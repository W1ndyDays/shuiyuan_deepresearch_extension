...
const DEFAULTS = {
  ...
  maxReadLength: 600,
  maxResults: 8,
  llmTimeoutMs: undefined, // undefined -> lib/llm.mjs default
};
    ...
    user: `研究主题：${topic}\n\n以下是论坛原始材料：\n\n${material}\n\n输出 JSON：
{
  ...
  "followup_queries": ["从材料中发掘的新搜索词（0-4个，必须来自材料中出现的线索）"]
    ...
    onRetry: (info) => emit({ type: "llm-retry", phase: "agent", ...info }),
  });
...
{"queries": [{"q": "搜索词", "angle": "该查询覆盖的角度"}]}`,
      ...
      onRetry: (info) => emit({ type: "llm-retry", phase: "plan", ...info }),
    });
  } catch (err) {
...
${topicsList || "  (无)"}`;
  ...
  let degraded = false;
  try {
    report = await chatTurn(llm, {
      system: CHAT_SYSTEM,
      messages,
      maxTokens: 4000,
      signal,
      timeoutMs: opts.llmTimeoutMs,
      onRetry: (info) => emit({ type: "llm-retry", phase: "synthesis", ...info }),
    });
  } catch (err) {
    if (err?.name === "AbortError") throw err;
    // Everything up to here cost minutes of forum reads and one LLM call per
    // agent. Losing all of it because the final call failed is the worst
    // possible outcome, so fall back to a locally assembled digest: real links,
    // no extra tokens, and the session stays answerable.
    report = localReport(topic, agentResults, err);
    degraded = true;
    emit({ type: "synthesis-fallback", error: err?.message || String(err) });
  }
...
/** Deterministic stand-in for the LLM report when the synthesis call fails. */
function localReport(topic, agentResults, err) {
  const seen = new Set();
  const topics = [];
  for (const r of agentResults) {
    for (const t of r.relevantTopics) {
      const url = t.url || (t.id != null ? `${SITE}/t/${t.id}` : null);
      if (!url || seen.has(url)) continue;
      seen.add(url);
      topics.push(`- [${t.title || url}](${url}) — ${t.why || ""}`);
    }
  }
  const findings = agentResults.flatMap((r) =>
    r.findings.map((f) => `- ${f}（来自查询"${r.query}"）`),
  );
  return [
    `## 总览`,
    `综合报告生成失败（${err?.message || String(err)}），以下是未经模型综合的原始调研材料汇总。`,
    ``,
    `## 主要发现`,
    findings.join("\n") || "（无）",
    ``,
    ...
    `## 各查询角度总结`,
    agentResults
      .map((r) => `**${r.query}**（${r.angle || "—"}）：${r.summary || "（无）"}`)
      .join("\n\n"),
    ``,
    `> 提示：可以直接在下方追问，我会基于这些材料回答；也可以重新发起一次深度搜索以获得正式报告。`,
  ].join("\n");
}
...
const FOLLOWUP_DEFAULTS = {
  ...
  postsPerTopic: 4,
  maxReadLength: 500,
  // Append-only history eventually outgrows the model's context window; past
  // this many characters we drop the oldest middle turns (see trimHistory).
  maxHistoryChars: 160_000,
  llmTimeoutMs: undefined, // undefined -> lib/llm.mjs default
};

/**
 * Keep the conversation inside the model's context window.
 * The first user turn (all the research material) and the report are what every
 * later answer is grounded in, so they stay; the oldest follow-up turns in the
 * middle are dropped in whole (assistant+user) pairs until we're under budget.
 * Returns the number of dropped turns so the caller can tell the user.
 */
function trimHistory(messages, maxChars) {
  ...
  const KEEP_HEAD = 2; // materials + first report
  const KEEP_TAIL = 4; // the most recent exchanges
  let dropped = 0;
  while (size() > maxChars && messages.length > KEEP_HEAD + KEEP_TAIL) {
    messages.splice(KEEP_HEAD, 1);
    dropped += 1;
  }
  return dropped;
}
...
export async function followUp({ question, conversation, llm, sy, onEvent, signal, options = {} }) {
  ...
  // Trim BEFORE recording baseLength so a failed turn rolls back to the trimmed
  // history rather than resurrecting turns we deliberately dropped.
  const dropped = trimHistory(messages, opts.maxHistoryChars);
  if (dropped) emit({ type: "chat-trimmed", dropped });
  ...
  const ask = async () => {
    ...
    return chatTurn(llm, {
      ...
      signal,
      timeoutMs: opts.llmTimeoutMs,
      onRetry: (info) => emit({ type: "llm-retry", phase: "chat", ...info }),
    });
  };
  ...
  try {
    ...
    // Last resort: a stubborn model can answer even the forced instruction with
    // another directive. Never let raw JSON become the visible answer.
    if (parseSearchDirective(answer, opts.queriesPerRound)) {
      answer = searched.length
        ? `我已经就「${searched.join("、")}」搜索过论坛，但仍未能整理出可靠的回答。建议换个说法再问一次，或直接对这些关键词发起一次新的深度搜索。`
        : "抱歉，我没能整理出可靠的回答。建议换个说法再问一次。";
    }
